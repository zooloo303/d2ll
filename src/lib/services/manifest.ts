...
import type { ManifestResponse, ManifestTableName, ManifestTable, VersionMetadata, TableMetadata } from '$lib/utils/types';
...
async function storeTable(tableName: string, data: ManifestTable): Promise<void> {
    const db = await openDatabase();
...
export async function updateManifest(progressCallback?: (current: number, total: number) => void): Promise<void> {
    ...
    const tables = Object.entries(metadata.Response.jsonWorldComponentContentPaths.en);
...
export async function getManifestTable<T extends ManifestTableName>(tableName: T): Promise<ManifestTable | null> {
